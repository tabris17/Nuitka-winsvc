...
DWORD SvcInstall(LPCWSTR cmdLine)
{
    ...
    SC_HANDLE serviceHandle = CreateServiceW(
        scManagerHandle,
        _NUITKA_WINSVC_NAME_UNICODE,
        _NUITKA_WINSVC_DISPLAY_NAME_UNICODE,
        SERVICE_ALL_ACCESS,
    ...
    #ifdef _NUITKA_WINSVC_DESCRIPTION_UNICODE
    SERVICE_DESCRIPTIONW serviceDescription;
    serviceDescription.lpDescription = _NUITKA_WINSVC_DESCRIPTION_UNICODE;
...
DWORD SvcUninstall()
{
    ...
    SC_HANDLE serviceHandle = OpenServiceW(scManagerHandle, _NUITKA_WINSVC_NAME_UNICODE, DELETE);
...
{
    svcStatusHandle = RegisterServiceCtrlHandlerW(_NUITKA_WINSVC_NAME_UNICODE, SvcCtrlHandler);
    if (!svcStatusHandle) {
        return;
    }
...
int wmain(int argc, wchar_t** argv)
{
    if (argc == 2) {
        if (wcscmp(argv[1], L"install") == 0) {
            ...
            #ifdef _NUITKA_WINSVC_CMDLINE_UNICODE
            wchar_t cmdLine[2 << 12];
            swprintf_s(cmdLine, _countof(cmdLine), L"%s %s", imagePath, _NUITKA_WINSVC_CMDLINE_UNICODE);
    ...
    SERVICE_TABLE_ENTRYW serviceTable[] = {
        { (LPWSTR)_NUITKA_WINSVC_NAME_UNICODE, (LPSERVICE_MAIN_FUNCTIONW)SvcMain },
        { NULL, NULL }
    };
